...
namespace GameLab.GridSystem
{
    public class HexGridFeatureManager : MonoBehaviour
    {
        ...
        void AddWallSegment(Vector3 nearLeft, Vector3 farLeft, Vector3 nearRight, Vector3 farRight, bool addTower = false)
        {
            ...
            nearRight = HexMetric.Perturb(nearRight);
            farRight = HexMetric.Perturb(farRight);
            Vector3 left = HexMetric.WallLerp(nearLeft, farLeft);
            Vector3 right = HexMetric.WallLerp(nearRight, farRight);


            Vector3 leftThicknessOffset = HexMetric.WallThicknessOffset(nearLeft, farLeft);
            Vector3 rightThicknessOffset = HexMetric.WallThicknessOffset(nearRight, farRight);

            float leftTop = left.y + HexMetric.wallHeight;
            float rightTop = right.y + HexMetric.wallHeight;
        ...
        void AddWallSegment(Vector3 pivot, HexCell pivotCell, Vector3 left, HexCell leftCell, Vector3 right, HexCell rightCell)
        {
            ...
            {
                if (rightCell.GetElevation() < leftCell.GetElevation())
                {
        ...
        {
            near = HexMetric.Perturb(near);
            far = HexMetric.Perturb(far);

            Vector3 center = HexMetric.WallLerp(near, far);
            Vector3 thickness = HexMetric.WallThicknessOffset(near, far);

            Vector3 v1, v2, v3, v4;

            v1 = v3 = center - thickness;
            v2 = v4 = center + thickness;
            v3.y = v4.y = center.y + HexMetric.wallHeight;
            walls.AddQuadUnperturbed(v1, v2, v3, v4);
        }
        ...
        void AddWallWedge(Vector3 near, Vector3 far, Vector3 point)
        {
            ...
            v1 = v3 = center - thickness;
            v2 = v4 = center + thickness;
            v3.y = v4.y = pointTop.y = center.y + HexMetric.wallHeight;
